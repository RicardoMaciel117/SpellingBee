...
var attempts = 3, category="", word, noBtns = 28,answer="";
...
function setCategory(cat){
	if(cat=="BASIC"){
    	word = basicWords[Math.round(Math.random() * (basicWords.length - 2))];
  	}
  	else if(cat=="INTERMEDIATE"){
    	word = intermediateWords[Math.round(Math.random() * (intermediateWords.length - 2))];	
    }
  	else if(cat=="ADVANCED"){
    	word = advancedWords[Math.round(Math.random() * (advancedWords.length - 2))];	
    }
    else{
    	setCategory(randomCategories[Math.round(Math.random() * (randomCategories.length - 1))]);
 		return;   
    }
    category = cat;
    alert(word);
...
function generateLetters(){
    ...
    for (var i = 1;  i <= noBtns; i++) {
      document.getElementById('_'+i).value = randomletters[Math.round(Math.random() * (25 - 0))];
    }
}
